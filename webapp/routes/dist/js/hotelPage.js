...
hotelService.on("touchend", function (e) {
    ...
    if ($target.is("li img:first-child")) {
        ...
        servicesBigs.each(function (num) {
            servicesBigs.eq(num).attr("src", __WEBPACK_IMPORTED_MODULE_0__data_js__["a" /* default */].serviceData[num].img[0]);
        });
        $target.attr("src", __WEBPACK_IMPORTED_MODULE_0__data_js__["a" /* default */].serviceData[index].img[1]);
        serviceInfoInfo.html(__WEBPACK_IMPORTED_MODULE_0__data_js__["a" /* default */].serviceData[index].text);
        serviceInfoPrice.html(__WEBPACK_IMPORTED_MODULE_0__data_js__["a" /* default */].serviceData[index].price);
    }
});
//根据分数控制表格的显示长度
let uppers = $(".upper");
let commentScoresDisplay = $(".chart span");
__WEBPACK_IMPORTED_MODULE_0__data_js__["a" /* default */].commentScore.forEach(function (item, index) {
