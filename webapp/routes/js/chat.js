...
$iSend.on("touchend",function(){
    var newTalk = $("<div class='customer-talk'></div>").html("<div><p>"+$iTalk.val()+"</p><p class='talk-time'><strong>"+tabDate(new Date()).date+"</strong><em>"+tabDate(new Date()).time+"</em></p></div>");
    $(".wrap").append(newTalk);
    $iTalk.val("");
});
//封装转换为日期的函数
